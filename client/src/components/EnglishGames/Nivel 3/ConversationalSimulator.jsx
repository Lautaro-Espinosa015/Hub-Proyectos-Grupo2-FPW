...
// Define la estructura de la conversación como un objeto donde cada clave es un ID de diálogo.
// Cada diálogo contiene el texto y audio del personaje, y un array de opciones para el usuario.
...
export default function ConversationalSimulator() {
  ...
  // --- P3: Manejo de Audio ---
  // Función para reproducir un audio, asegurando que solo uno suene a la vez
  ...
  }, []); // Dependencias vacías, esta función no necesita recrearse
  ...
  useEffect(() => {
    ...
    if (dialogue) {
      ...
      playSound(dialogue.characterAudio, () => {
        ...
        // Si es un diálogo final sin opciones, podemos hacer una transición o mostrar un mensaje
        if (dialogue.options.length === 0 && !dialogue.isFinal) {
          // Este es el final de una rama de conversación, pero no el final del juego.
          // Podríamos, por ejemplo, pasar al estado 'end_game' automáticamente.
